...
with tab1:
    ...
    with col1:
        ...
        if prompt := st.chat_input(
            "Ask anything - Router Agent will direct to the right specialist..."
        ):
...
with tab3:
    ...
    card_tabs = st.tabs(
        ["Active Cards", "Issue New Card", "Transactions", "Limits & Controls"]
    )
        ...
        with st.form("new_card_form"):
            ...
            with col2:
                ...
                transaction_limit = st.number_input(
                    "Per Transaction Limit ($)", 100, 10000, 1000
                )
...
with tab5:
    ...
    with qb_tabs[2]:  # Reports
        ...
        report_type = st.selectbox(
            "Select Report", ["Profit & Loss", "Balance Sheet", "Cash Flow Statement"]
        )
...
with tab6:
    ...
    if analysis_type == "Trend Analysis":
        # Get historical data
        transactions = st.session_state.data_service.get_transaction_history(
            selected_org, 180
        )
        if transactions:
